...
class TensForm(forms.ModelForm):
    ...
    def clean_Registro_tens(self):
        registro = self.cleaned_data.get('Registro_tens')
        
        if self.instance and self.instance.pk:
            if Tens.objects.filter(Registro_tens=registro).exclude(pk=self.instance.pk).exists():
                raise ValidationError('Este número de registro ya existe.')
        else:
            if Tens.objects.filter(Registro_tens=registro).exists():
                raise ValidationError('Este número de registro ya existe.')
