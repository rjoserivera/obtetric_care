...
class IngresoPaciente(models.Model):
    ...
    def save(self, *args, **kwargs):
        """Generar número de ficha automáticamente si no existe"""
        if not self.numero_ficha:
            # Formato: ING-YYYYMMDD-ID_PACIENTE
            now = timezone.now()
            self.numero_ficha = f"ING-{now.strftime('%Y%m%d')}-{self.paciente.pk}"
            
            # Verificar unicidad y agregar sufijo si es necesario
            count = 1
            original_num = self.numero_ficha
            while IngresoPaciente.objects.filter(numero_ficha=self.numero_ficha).exists():
                self.numero_ficha = f"{original_num}-{count}"
                count += 1
...
class FichaObstetrica(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.numero_ficha:
            # Generar número de ficha: FO-AÑO-MES-DIA-RUT_PACIENTE (o algo único)
            # Usaremos un timestamp simple + ID paciente para unicidad
            now = timezone.now()
            self.numero_ficha = f"FO-{now.strftime('%Y%m%d')}-{self.paciente.pk}"
            
            # Verificar si ya existe (por si acaso)
            count = 1
            original_num = self.numero_ficha
            while FichaObstetrica.objects.filter(numero_ficha=self.numero_ficha).exists():
                self.numero_ficha = f"{original_num}-{count}"
                count += 1
