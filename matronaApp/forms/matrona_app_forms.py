...
class FichaObstetricaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'patologias_criticas': forms.Select(attrs={
                'class': 'form-select'
            }),
...
class IngresoPacienteForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Filtrar solo pacientes activos y ordenar alfabéticamente
        self.fields['paciente'].queryset = Paciente.objects.filter(
            activo=True
        ).select_related('persona').order_by(
            'persona__Apellido_Paterno',
            'persona__Apellido_Materno',
            'persona__Nombre'
        )
        
        # Mejorar el label del selector
        self.fields['paciente'].label_from_instance = lambda obj: (
            f"{obj.persona.Rut} - "
            f"{obj.persona.Apellido_Paterno} {obj.persona.Apellido_Materno}, "
            f"{obj.persona.Nombre}"
        )
    
    def clean_paciente(self):
        """Validar que la paciente no tenga un ingreso activo"""
        paciente = self.cleaned_data.get('paciente')
        
        if paciente:
            # Verificar si ya tiene un ingreso activo
            ingreso_activo = IngresoPaciente.objects.filter(
                paciente=paciente,
                activo=True
            ).first()
            ...
            if ingreso_activo:
                raise ValidationError(
                    f'Esta paciente ya tiene un ingreso activo (Ficha: {ingreso_activo.numero_ficha}). '
                    f'Debe dar de alta el ingreso anterior antes de registrar uno nuevo.'
                )
